...
const cg_miner_api_1 = require("../src/cg-miner-api");
...
async function testMiner(miner) {
    console.log(`\n🔍 Testing ${miner.name} (${miner.ip}:${miner.port})...`);
    try {
        // Test version
        const versionResult = await cg_miner_api_1.CGMinerAPI.aioVersion(miner.ip, miner.port);
        if (versionResult.isRequestSuccess()) {
            ...
            // Get summary
            const summaryResult = await cg_miner_api_1.CGMinerAPI.summary(miner.ip, miner.port);
            ...
            // Get pools
            const poolsResult = await cg_miner_api_1.CGMinerAPI.pools(miner.ip, miner.port);
...
async function main() {
    ...
    const results = await Promise.all(MINERS.map((miner) => testMiner(miner)));
...
//# sourceMappingURL=test-miners.js.map